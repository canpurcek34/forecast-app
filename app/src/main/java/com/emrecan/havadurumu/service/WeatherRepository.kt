...
import com.emrecan.havadurumu.BuildConfig
import com.emrecan.havadurumu.BuildConfig.API_KEY
...
class WeatherRepository @Inject constructor(
    ...
    @RequiresPermission(ACCESS_FINE_LOCATION)
    fun getHourlyForecast(): Flow<List<FullWeather.Hourly>> {
        return locationFlow().map {
            service.getHourlyWeather(it.latitude, it.longitude, BuildConfig.API_KEY)
                .body()
        }.map {
            it?.hourly?.drop(1)?.take(10)
        }.filterNotNull()
    }
